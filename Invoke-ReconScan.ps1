<#
.SYNOPSIS
    Simulacao de atividade de reconhecimento (port scanning) para validar o IPS
    do Cisco Secure Access em ambiente de POC/laboratorio.

.DESCRIPTION
    Executa tres tecnicas classicas de reconnaissance que tipicamente disparam
    assinaturas Snort 3 da familia INDICATOR-SCAN / PROTOCOL-SCAN no IPS do
    Cisco Secure Access:

      1. ICMP Sweep   - varredura de hosts vivos em uma faixa de IPs.
      2. TCP Port Scan - varredura paralela em uma lista de portas comuns.
      3. Sequential Scan - varredura sequencial em uma faixa de portas, padrao
         classico de ferramentas tipo nmap -p 1-1024.

    O script gera um log local com timestamps para correlacionar com os eventos
    que aparecerao no Activity Search do Secure Access.

    PRE-REQUISITO: o trafego desta maquina precisa atravessar o Secure Access
    (Secure Client com Internet Security, tunnel IPsec/GRE, ou Resource
    Connector com IPS aplicado). Caso contrario, nada sera inspecionado.

.PARAMETER Target
    Alvo do scan. Pode ser hostname ou IP. Recomendado: scanme.nmap.org ou
    uma VM publica de sua propriedade.

.PARAMETER Mode
    Tecnica de scan. Valores: All, IcmpSweep, TcpScan, SequentialScan.
    Default: All (executa as tres em sequencia).

.PARAMETER Ports
    Lista de portas TCP para o modo TcpScan. Default: 30 portas comuns.

.PARAMETER SequentialStart
    Porta inicial do modo SequentialScan. Default: 1.

.PARAMETER SequentialEnd
    Porta final do modo SequentialScan. Default: 1024.

.PARAMETER IcmpRangeStart
    Primeiro octeto final da faixa ICMP. Default: 1.

.PARAMETER IcmpRangeEnd
    Ultimo octeto final da faixa ICMP. Default: 50.

.PARAMETER TimeoutMs
    Timeout em milissegundos para cada tentativa TCP. Default: 400ms.

.PARAMETER Concurrency
    Numero de threads paralelas. Quanto maior, mais "barulhento" e maior a
    chance de disparar threshold do IPS. Default: 50.

.PARAMETER LogPath
    Caminho do arquivo de log. Default: .\recon-scan-<timestamp>.log

.EXAMPLE
    .\Invoke-ReconScan.ps1 -Target scanme.nmap.org

    Executa as tres tecnicas contra o alvo de teste oficial do Nmap.

.EXAMPLE
    .\Invoke-ReconScan.ps1 -Target 203.0.113.10 -Mode TcpScan -Concurrency 100

    Apenas TCP scan agressivo contra a VM publica do laboratorio.

.EXAMPLE
    .\Invoke-ReconScan.ps1 -Target 203.0.113.10 -Mode SequentialScan `
        -SequentialStart 1 -SequentialEnd 2048

    Scan sequencial de portas baixas (padrao classico de reconhecimento).

.NOTES
    Autor   : POC Cisco Secure Access
    Uso     : Apenas em laboratorio controlado e contra alvos autorizados.
    AVISO   : Executar scan de portas contra alvos sem autorizacao explicita
              e ilegal na maioria das jurisdicoes.
#>
